    ...
    <main className="min-h-screen bg-[#f3f5f8] text-[#08163F]">
      <div className="grid min-h-screen lg:grid-cols-[300px_1fr]">
        <aside className="hidden border-r border-slate-100 bg-white p-5 lg:block">
          ...
          <nav className="mt-8 space-y-2">
            ...
            <MenuItem label="Meus módulos" ativo />
          ...
          <div className="mt-8 rounded-[1.5rem] bg-gradient-to-br from-[#07122F] via-[#0A1E55] to-[#12317C] p-5 text-white">
        ...
        <section className="flex min-w-0 flex-col">
          <header className="sticky top-0 z-20 flex min-h-[78px] flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-white/85 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <h2 className="line-clamp-1 text-lg font-black sm:text-xl md:text-2xl">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-[#08163F] shadow-sm"
              >
                Suporte
              </button>
              ...
              <button
                onClick={sair}
                className="rounded-2xl bg-[#08163F] px-4 py-3 text-sm font-black text-white shadow-lg"
              >
                Sair
              </button>
            </div>
          ...
          <div className="grid gap-6 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_380px] lg:p-8">
            {erro && (
            ...
            {modulosDisponiveis.length === 0 ? (
              <section className="lg:col-span-2">
                <div className="rounded-[2rem] border border-dashed border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/70 sm:p-10">
                  <h1 className="text-2xl font-black">Nenhum módulo disponível ainda</h1>
                  <p className="mx-auto mt-3 max-w-xl text-sm font-semibold leading-6 text-slate-500">
                    Assim que a mentora cadastrar módulos, aulas e atividades, eles aparecerão aqui.
                  </p>
                </div>
              </section>
            ) : (
              <>
                <section className="min-w-0 space-y-6">
                  {moduloSelecionado && (
                    <section className="overflow-hidden rounded-[26px] bg-gradient-to-br from-[#07122F] via-[#0A1E55] to-[#12317C] p-5 text-white shadow-2xl shadow-[#07122F]/20 sm:p-6">
                      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                        <div className="min-w-0">
                          ...
                          <p className="mt-4 max-w-3xl text-sm font-semibold leading-7 text-[#D9DEE7]">
                            {getModuloDescricaoCurta(moduloSelecionado) ||
                              "Este módulo ainda não possui descrição curta."}
                          </p>
                        </div>

                        <div className="w-full rounded-[24px] border border-white/15 bg-white/10 p-4 backdrop-blur-sm xl:max-w-[280px]">
                  ...
                  <section className="grid gap-4 md:grid-cols-2">
                    ...
                    <InfoModuloCard
                      ...
                      texto={
                        moduloSelecionado?.atividade_pratica ||
                        "A atividade prática ainda não foi cadastrada."
                      }
                    />
                  </section>

                  <section className="overflow-hidden rounded-[26px] bg-gradient-to-br from-[#07122F] via-[#0A1E55] to-[#12317C] shadow-2xl shadow-[#07122F]/20">
                    {embedUrl ? (
                      <iframe
                        src={embedUrl}
                  ...
                  <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                    <div>
                      <h2 className="max-w-3xl text-2xl font-black leading-tight text-[#050816] sm:text-3xl">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className="rounded-full border border-gray-200 bg-white px-6 py-4 font-black text-[#08163F] shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
                      >
                      ...
                      {aulaSelecionada && (
                        <button
                          ...
                          className={`rounded-full px-6 py-4 font-black shadow-lg transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 ${
                ...
                <aside className="min-w-0">
                  <section className="rounded-[26px] bg-white p-4 shadow-xl shadow-slate-200/70 sm:p-5">
                    <div className="rounded-[1.5rem] bg-gradient-to-br from-[#07122F] via-[#0A1E55] to-[#12317C] p-5 text-white sm:p-6">
                      <p className="text-xs font-black uppercase tracking-[0.25em] text-blue-200">
                        Progresso do curso
                      </p>

                      <strong className="mt-5 block text-3xl font-black sm:text-4xl">
                        {aulasConcluidas.length}/{totalAulas}
                      </strong>
                    ...
                    <div className="mt-5 max-h-[calc(100vh-310px)] space-y-4 overflow-y-auto pr-2">
                      {modulosDisponiveis.map((modulo) => {
                        ...
                        return (
                          <div key={modulo.id} className="overflow-hidden rounded-[1.5rem] border border-slate-100">
                            <button
                              type="button"
      ...
      {modalArquivosAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-4xl rounded-[24px] bg-white p-5 shadow-2xl sm:rounded-[34px] sm:p-7">
            <div className="mb-7 flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="text-2xl font-black text-[#050816] sm:text-4xl">
                  Arquivos disponíveis
                </h2>
              </div>
            ...
            {arquivosDaAula.length > 0 ? (
              <div className="space-y-4">
                {arquivosDaAula.map((arquivo) => (
                  <div key={arquivo.id} className="flex flex-col gap-4 rounded-[26px] bg-[#f9fafb] p-5 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                    ...
                    <a href={arquivo.url} target="_blank" rel="noreferrer" className="rounded-full bg-gradient-to-b from-[#F3F4F6] via-[#D1D5DB] to-[#9CA3AF] px-8 py-4 text-center font-black text-[#08163F] shadow-lg transition hover:brightness-105">
                      Abrir material
                    </a>
                  </div>
                ))}
...
function InfoModuloCard({ titulo, texto }: { titulo: string; texto: string }) {
  return (
    <article className="rounded-[24px] bg-white p-5 shadow-xl shadow-slate-200/70">
...
function InfoAulaCard({
  ...
  return (
    <article className="w-full rounded-[24px] bg-white p-5 shadow-xl shadow-slate-200/70 sm:p-6">
      <div className="grid gap-6 xl:grid-cols-[1fr_auto] xl:items-center">
        <div className="min-w-0">
          ...
          <h3 className="mt-3 text-2xl font-black leading-tight text-[#08163F] sm:text-3xl">
            {titulo}
          </h3>

          <p className="mt-4 max-w-none whitespace-pre-line text-sm font-semibold leading-7 text-slate-500 sm:text-base">
            {texto}
          </p>
        </div>
...
function StatusMentoradoBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-xs font-black ${classes[status] || "bg-slate-100 text-slate-600"}`}>
      {status}
    </span>
  );
}
