...
export default function Sidebar({ nome }: SidebarProps) {
  ...
  return (
    <aside className="relative flex min-h-screen w-[290px] flex-col overflow-hidden border-r border-white/10 bg-gradient-to-b from-[#07122F] via-[#0A1E55] to-[#12317C] p-5 text-white shadow-[0_18px_50px_rgba(8,22,63,0.22)]">
      <div className="pointer-events-none absolute -top-16 -right-16 h-44 w-44 rounded-full bg-[radial-gradient(circle,rgba(229,231,235,0.16),transparent)]" />
      <div className="pointer-events-none absolute bottom-10 -left-14 h-36 w-36 rounded-full bg-[radial-gradient(circle,rgba(191,195,201,0.10),transparent)]" />
      ...
      <div className="relative z-10">
        ...
        <div className="mb-6 rounded-[22px] border border-white/10 bg-white/8 p-4 backdrop-blur-sm">
          ...
          <p className="mt-1 text-sm text-[#D9DEE7]">Mentora</p>
        </div>
        ...
        <nav className="space-y-2">
          {menusMentora.map((item) => (
            <button
              ...
              className="flex w-full items-center justify-between rounded-2xl border border-transparent bg-white/6 px-4 py-3 text-left text-sm font-medium text-[#E5E7EB] transition hover:border-white/10 hover:bg-white/12 hover:text-white"
            >
              <span>{item.label}</span>
              <span className="text-[#BFC3C9]">→</span>
            </button>
          ))}
        </nav>
      </div>
